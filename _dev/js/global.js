...
(function () {
    ...
    const settings = {
        ...
        itemEl: '.menu__item',
        linkEl: '.menu__link',
        linkElHasSubmenu: 'menu__link--has-submenu',
        submenuElCurrent: 'menu__link--current',
        submenuEl: 'menu__submenu',
        submenuElOpen: 'menu__submenu--open'
    }

    // Get menu el
    let menu = document.querySelector(settings.menuEl);

    // Add menu role 'menubar'
    menu.setAttribute('role', 'menubar');

    // All menu items
    let menuItems = menu.querySelectorAll(settings.itemEl);
    ...
    Array.prototype.forEach.call(menuItems, (menuItem) => {
        ...
        // Get menu link el
        let menuLink = menuItem.querySelector(settings.linkEl);

        // Add link role 'menuitem'
        menuLink.setAttribute('role', 'menuitem');

        // Check if item has submenu
        if (menuLink.classList.contains(settings.linkElHasSubmenu)) {

            // Add item aria attr (if it has submenu)
            menuLink.setAttribute('aria-haspopup', true);
            menuLink.setAttribute('aria-expanded', false);

            // Get submenu el
            let submenu = menuItem.querySelector('.' + settings.submenuEl);

            // Add submenu role 'menu'
            submenu.setAttribute('role', 'menu');

            // Get all submenu's links
            let submenuLinks = submenu.querySelectorAll(settings.linkEl);

            // Each submenu's links
            Array.prototype.forEach.call(submenuLinks, (submenuLink) => {
                submenuLink.setAttribute('tabindex', '-1');
            });

            // If item is item 'level-1'
            if (!menuItem.parentNode.classList.contains(settings.submenuEl)) {

                if (window.matchMedia("(min-width: 1100px)").matches) {
                    menuLink.addEventListener('mouseenter', () => {
                        openMenu(menuLink, submenu);
                    });

                    menuLink.addEventListener('mouseleave', () => {
                        closeMenu(menuLink, submenu);
                    });
        ...
        if (menuLink.classList.contains(settings.submenuElCurrent)) {
    ...
    function openMenu(link, submenu) {
        ...
        submenu.classList.add(settings.submenuElOpen);
    }

    // close menu
    function closeMenu(link, submenu) {
        link.setAttribute('aria-expanded', false);
        submenu.classList.remove(settings.submenuElOpen);
    }
}());
...
(function () {
    ...
    const settings = {
        navMobile: '#navMain',
        navMobileBtn: '#navMainBtn',
        navMobileOpen: 'nav-main--open',
    }

    // Get nav mobile btn
    let menu = document.querySelector(settings.navMobile);

    // Get nav mobile btn
    let btn = document.querySelector(settings.navMobileBtn);
    ...
    btn.addEventListener('click', (e) => {
        ...
        if (expanded) {
            ...
            menu.classList.add(settings.navMobileOpen);
            scrollLock();
        } else {
            btn.setAttribute('aria-expanded', true);
            menu.classList.remove(settings.navMobileOpen);
            scrollUnLock();
        }
    });
...
(function (global) {
  ...
  A11yDialog.prototype.show = function (event) {
    ...
    this.node.classList.remove('modal--close');
  ...
  A11yDialog.prototype.hide = function (event) {
    ...
    this.node.classList.add('modal--close');
...
function scrollLock() {
    ...
    rootEl.style.paddingRight = scrollWidth + 'px';
}
